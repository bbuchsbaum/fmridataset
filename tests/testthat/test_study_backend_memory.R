test_that("StudyBackendSeed provides true lazy evaluation", {
  ...
  seed <- DelayedArray::seed(da)
  subset_mat <- if (inherits(seed, "StudyBackendSeed")) {
    DelayedArray::extract_array(seed, list(150:250, 1:10))
  } else {
    fmridataset:::extract_array.study_backend_seed(seed, list(150:250, 1:10))
  }
  ...
  expect_equal(unique(subset_mat[1:51, 1]), 2)   # Rows 150-200 (subset rows 1-51)
  expect_equal(unique(subset_mat[52:101, 1]), 3) # Rows 201-250 (subset rows 52-101)
})
...
test_that("StudyBackendSeed respects memory bounds", {
  ...
  registerS3method("as_delayed_array", "mock_large_backend", function(backend, sparse_ok = FALSE, ...) {
