...
create_file_dataset_simulation <- function() {
  # Simulates file dataset creation (no actual files needed)
  list(
    files = c("run1.nii", "run2.nii"),
    mask = "mask.nii",
    backend_created = Sys.time(),
    data_loaded = NULL
  )
}

simulate_data_access <- function(dataset_sim) {
  # Simulates first data access triggering loading
  dataset_sim$data_loaded <- Sys.time()
  dataset_sim$data_size_mb <- 150  # Simulated data size
  return(dataset_sim)
}

# Demonstrate lazy evaluation timing
file_sim <- create_file_dataset_simulation()
cat("Dataset created at:", format(file_sim$backend_created), "\n")
cat("Data loaded:", is.null(file_sim$data_loaded), "\n")

# First access triggers loading
file_sim <- simulate_data_access(file_sim)
cat("Data loaded at:", format(file_sim$data_loaded), "\n")
cat("Loading delay:", 
    difftime(file_sim$data_loaded, file_sim$backend_created, units = "secs"), "seconds\n")
```
