...
# Same chunking interface for individual datasets
matrix_chunks <- data_chunks(matrix_ds, nchunks = 3)
cat("Matrix chunks created:", length(matrix_chunks), "\n")

# Study datasets organize data per-subject
cat("Study dataset subjects:", length(study_ds$subject_ids), "\n")
```
...
# For file-based datasets, storage operations are delegated to backends
# matrix_dataset uses direct data storage, but file datasets use backends
# Example with file dataset (using study_ds which has a backend):
if (!is.null(study_ds$backend)) {
  backend <- study_ds$backend
  cat("Study dataset uses backend:", class(backend)[1], "\n")
}

# Direct access to dataset dimensions
dataset_dims <- dim(get_data_matrix(matrix_ds))
cat("Dataset dimensions:", dataset_dims[1], "timepoints x", dataset_dims[2], "voxels\n")
```
...
```{r factory-example}
# Matrix input uses matrix_dataset constructor
matrix_input <- matrix(rnorm(500), nrow = 50, ncol = 10)
auto_dataset1 <- matrix_dataset(datamat = matrix_input, TR = 2.0, run_length = 50)
cat("Matrix input created:", class(auto_dataset1)[1], "\n")

# File paths would use fmri_dataset which automatically creates file backend
# file_input <- c("scan1.nii", "scan2.nii")
# auto_dataset2 <- fmri_dataset(scans = file_input, mask = "mask.nii",
#                               TR = 2.0, run_length = c(100, 100))
# cat("File input created:", class(auto_dataset2)[1], "\n")

# Different constructors optimize for different data sources
cat("Constructor functions provide appropriate backend selection\n")
```
...
cat("Voxel chunking: ", voxel_chunks$nchunks, "chunks\n")

# Strategy 2: Run-based chunking
run_chunks <- data_chunks(matrix_ds, runwise = TRUE)
cat("Run chunking: ", run_chunks$nchunks, "chunks\n")

# Strategy 3: Single chunk (no subdivision)
single_chunk <- data_chunks(matrix_ds, nchunks = 1)
cat("Single chunking: ", single_chunk$nchunks, "chunks\n")

# Each strategy optimizes for different use cases
# Iterate through chunks using nextElem()
cat("\nVoxel chunk details:\n")
chunk1 <- voxel_chunks$nextElem()
cat("  Chunk 1:", ncol(chunk1$data), "voxels\n")
chunk2 <- voxel_chunks$nextElem()
cat("  Chunk 2:", ncol(chunk2$data), "voxels\n")

# Run-based chunks contain all voxels for specific runs
cat("\nRun chunk details:\n")
run_chunk1 <- run_chunks$nextElem()
cat("  Run 1:", nrow(run_chunk1$data), "timepoints,", ncol(run_chunk1$data), "voxels\n")
run_chunk2 <- run_chunks$nextElem()
cat("  Run 2:", nrow(run_chunk2$data), "timepoints,", ncol(run_chunk2$data), "voxels\n")
```
...
variable_tr_sampling_frame <- function(tr_sequence, run_boundaries, ...) {
  ...
  base_sf <- fmrihrf::sampling_frame(
    blocklens = run_boundaries,
...
benchmark_chunking <- function(dataset, chunk_sizes = c(1, 5, 10, 20)) {
  ...
  for (i in seq_along(chunk_sizes)) {
    ...
    start_time <- Sys.time()
    chunk_iter <- data_chunks(dataset, nchunks = n_chunks)

    # Simulate processing by iterating through chunks
    tryCatch({
      for (j in 1:chunk_iter$nchunks) {
    ...
    # Estimate peak memory: dataset size divided by number of chunks
    n_voxels <- if (!is.null(dataset$datamat)) ncol(dataset$datamat) else 1000
    results$memory_peak[i] <- n_timepoints(dataset) * n_voxels / n_chunks * 8 / 1024^2
  }
...
- **Matrix backends**: Suitable for datasets under 8GB with frequent random access patterns
- **File backends**: Optimal for large datasets (>8GB) with sequential access patterns
- **Study backends**: Required for multi-subject analyses with subject-level lazy loading
