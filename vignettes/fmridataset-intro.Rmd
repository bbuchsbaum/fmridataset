...
# Add some realistic signal structure
# Simulate activation in first 100 voxels during certain timepoints
activation_periods <- c(20:30, 50:60, 120:130, 150:160)
fmri_matrix[activation_periods, 1:100] <- 
  fmri_matrix[activation_periods, 1:100] + 0.5
...
events <- data.frame(
  onset = c(20, 50, 120, 150),        # Event start times in seconds
  duration = c(10, 10, 10, 10),       # Event durations
  trial_type = c("task", "task", "task", "task"),
  run = c(1, 1, 2, 2)                 # Which run each event belongs to
)
...
```{r real-example-output, echo=FALSE}
# Expected output:
# fMRI Dataset (matrix_dataset)
# Data dimensions: 200 timepoints × 1000 voxels
# Temporal structure: 2 runs (100, 100 timepoints)
# TR: 2 seconds
# Total duration: 400 seconds
