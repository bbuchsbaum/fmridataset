...
must_haves:
  truths:
    ...
    - "Package installs from tarball in fresh R session"
  artifacts:
...
<task type="auto">
  <name>Task 2: Test fresh installation</name>
  <files></files>
  <action>
  Test that the package installs and loads correctly in a clean R session:

  ```bash
  # Get the tarball name
  TARBALL=$(ls fmridataset_*.tar.gz | head -1)

  # Test installation in vanilla R session
  R --vanilla -e "
    # Install from tarball (will fail on dependencies - expected)
    # Instead, just verify the tarball structure is valid
    untar('${TARBALL}', list = TRUE) |> head(20)
  "
  ```

  Note: Full installation test requires all dependencies (delarr, neuroim2, etc.) which are not on CRAN. Instead, verify:
  1. Tarball extracts correctly
  2. DESCRIPTION is valid
  3. NAMESPACE is valid

  Run: `R CMD INSTALL --build . 2>&1 | tail -10` to verify local installation still works.
  </action>
  <verify>
  Run: `R --vanilla -e "library(fmridataset); packageVersion('fmridataset')"` should print version
  </verify>
  <done>
  - Package tarball has valid structure
  - Package installs locally
  - Package loads and reports correct version
  </done>
</task>
