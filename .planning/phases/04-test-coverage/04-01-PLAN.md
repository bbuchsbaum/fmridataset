...
must_haves:
  truths:
    ...
    - "zarr_backend constructor validates zarr package availability"
  artifacts:
...
stopifnot(exists("create_test_zarr"))
stopifnot(exists("create_test_h5"))
stopifnot(exists("create_test_matrix"))
stopifnot(exists("create_test_mask"))

# Verify create_test_matrix actually works (no external deps)
mat <- create_test_matrix(n_time = 5, n_voxels = 10)
stopifnot(is.matrix(mat))
stopifnot(nrow(mat) == 5)
stopifnot(ncol(mat) == 10)

# Verify create_test_mask actually works (no external deps)
mask <- create_test_mask(n_voxels = 20, proportion_valid = 0.5)
stopifnot(is.logical(mask))
stopifnot(length(mask) == 20)

# Verify create_test_zarr works if zarr installed
if (requireNamespace("zarr", quietly = TRUE)) {
  tmp <- tempdir()
  zarr_path <- create_test_zarr(dims = c(2,2,2,3), path = file.path(tmp, "test_verify.zarr"))
  stopifnot(file.exists(zarr_path))
}

cat("All helper functions verified working\n")
```
  </verify>
  <done>helper-backends.R exists with all four helper functions defined and verified working via actual function calls</done>
</task>
...
**Note: This is a complex task covering 7 distinct test groups. Allow adequate time.**
...
2. Helper functions verified working via actual calls (not just existence checks)
3. `tests/testthat/test-zarr_backend.R` exists with comprehensive tests
4. `devtools::test(filter = "zarr_backend")` passes or skips gracefully
5. Coverage analysis shows improvement from 5% baseline
</verification>

<success_criteria>
- helper-backends.R provides reusable test data generators (verified by calling them)
