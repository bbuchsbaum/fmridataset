...
must_haves:
  ...
  key_links:
    ...
    - from: "tests/testthat/test-h5_backend.R"
      ...
      via: "uses create_test_h5() helper from Plan 01"
      pattern: "create_test_h5"
---
...
<helper_dependency>
**Note:** This plan uses `create_test_h5()` from `helper-backends.R` (created in Plan 01).

Before using the helper, verify it exists:
```r
# At start of test file or in first test
if (!exists("create_test_h5")) {
  skip("helper-backends.R not loaded - run Plan 01 first")
}
```

If Plan 01 and Plan 02 run in parallel (Wave 1), testthat will auto-load helper-backends.R
before test-h5_backend.R runs, since helper files are sourced before test files alphabetically.
</helper_dependency>
